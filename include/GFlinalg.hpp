...
/**
 * Basic GF element class.
 *
 * All math operations are done directly in polynomial form.
 *
...
public:
    ...
    */
    ...
    // �alculates the degree of the polynomial
    size_t degree(size_t startPos = 1) { return order - leadElemPos(value, startPos); }
    ...
    Returns inverse of polynomial
        ...
        a * a^(-1) == a^(-1) * a == 1
    */
    BasicBinPolynomial getInverse() { return pow(*this, order - 2); }
    /*
    Inverts polynomial
    ...
    */
    ...
    friend const T& operator+(const BasicBinPolynomial& a) { return a.value; }
...
/*
GF element class; uses conversion to power of primitive element Through a LUT internally
...
public:
    ...
    */
    ...
    Returns a pair of look-up vectors (LUTPair):
        ...
        polToInd: polynomial -> power of primitive element
    */
    static LUTPair getAlphaToIndex() { return alphaToIndex; }
    ...
    PowBinPolynomial& operator*=(const PowBinPolynomial& other) {
        this->val() = alphaToIndex.indToPol[alphaToIndex.polToInd[this->value] + alphaToIndex.polToInd[other.value]];
        return (*this);
    }
    ...
    PowBinPolynomial operator+(const PowBinPolynomial& other) const { return PowBinPolynomial(polSum(*this, other)); }
...
PowBinPolynomial<T, modPol> pow(const PowBinPolynomial<T, modPol>& val, size_t power) {
    return PowBinPolynomial<T, modPol>(
        PowBinPolynomial<T, modPol>::alphaToIndex
            .indToPol[(PowBinPolynomial<T, modPol>::alphaToIndex.polToInd[val.value] * power) % (val.gfOrder() - 1)]);
}
template <class T, T modPol>
class FastMultContainer {
public:
 ...
 * Basic GF element class.
 *
 * All math operations are done directly in the polynomial form.
 *
...
protected:
    ...
    /// @return The degree of the modulus polynomial
    uint8_t modPolDegree() {
        uint8_t pos           = 0;
        uint8_t containerSize = sizeof(T) << 3;
        for (uint8_t i = 1; i < containerSize + 1; ++i) {
            pos = containerSize - i;
            if ((mState.modPol >> pos) & 1)
                break;
        }
        return pos;
    }

    /// @return The position of the leading 1 in the polynomial from the left
    ...
    BasicGFElem polMulOld(const BasicGFElem& a, const BasicGFElem& b) const {
        BasicGFElem res(0, a.mState.modPol);

        for (size_t i = 0; i < a.mState.order; ++i) {
            if ((b.value >> i) & 1) {
                res.value ^= a.value << i;
            }
        }
    ...
    BasicGFElem polSum(const BasicGFElem& a, const BasicGFElem& b) const {
        return BasicGFElem(a.value ^ b.value, a.mState.modPol);
    }
    ...
    BasicGFElem polDiv(const BasicGFElem& a, const BasicGFElem& b) const {
        ...
        auto invB = pow(b, a.mState.order - 2);
...
public:
    ...
    explicit constexpr BasicGFElem(const T& value, const T& modulus, bool doReduce = true)
        : value(value), mState(modPolDegree(), 1 << SZ, modulus) {
        if (doReduce)
            reduce();
    }

    template <typename Iter>
    explicit constexpr BasicGFElem(Iter first, Iter last, const T& modulus)
        : value(0), mState(0, 0, modulus) {
        while (first != last) {
            value |= (static_cast<T>(*first) & 1);
            value <<= 1;
        }
    ...
    template <typename Iter>
    explicit constexpr BasicGFElem(Iter first, Iter last, Iter firstMod, Iter lastMod)
        : value(0), mState(0, 0, 0) {
     ...
     *
     * @warning Using this operator to alter contained value violates the internal invariant.
     *    Use the \c reduce() method instead.
     *
     *  @see reduce()
    */
    T& val() { return value; }
    ...
    /// @return \c mState.modPol
    T getMod() const { return mState.modPol; }

    /// Reduces the element by modulus polynomial (\c modPol)
    T reduce() {
    ...
    /**
     * @return For polynomial \c a <tt> a^(-1) </tt> such that <tt> a * a^(-1) == a^(-1) * a == 1</tt>
     */
    BasicGFElem getInverse() { return pow(*this, mState.order - 2); }

    /**
     * For polynomial \c a:
     * - Calculates <tt>a^(-1)</tt> such that <tt>a * a^(-1) == a^(-1) * a == 1</tt>
     * - Sets <tt>a = a^(-1)</tt>
    */
    BasicGFElem& invert() {
        *this = pow(*this, mState.order - 2);
        return *this;
    }
    ...
    size_t degree(size_t startPos = 1) const {
        return mState.order - leadElemPos(value, startPos);
    }
 ...
 * GF element class.
 *
 * Uses conversion to power of primitive element through a LUT internally.
 *
 * Uses a LUTPair pointer internally.
 * To make the LUTPair use <tt>LUTPair<T>::makeLUT(params...)</tt>.
...
public:
    struct LUTPair {
        ...
        LUTPair(const LUTPair&& lut) : polToInd(std::move(lut.polToInd)), indToPol(std::move(lut.indToPol)) {}
    };
...
public:
    ...
    explicit PowGFElem(const T& val, const T& modPol, LUTPair* lut)
        : BasicGFElem<T>(val, modPol), alphaToIndex(lut) { }

    explicit PowGFElem(const BasicGFElem<T>& pol, LUTPair* lut)
        : BasicGFElem<T>(pol), alphaToIndex(lut) {}

    /**
     * Creates a two-way look-up table for a specified field.
     * @return Pointer to generated \c LUTPair:
     *  - \c indToPol: power of primitive element -> polynomial
     *  - \c polToInd: polynomial -> power of primitive element
    */
    ...
    LUTPair* lut() const { return alphaToIndex; }

    /// Access the internall look-up table
    LUTPair* lut() { return alphaToIndex; }
...
PowGFElem<T> pow(const PowGFElem<T>& val, size_t power) {
    return PowGFElem<T>(val.alphaToIndex->indToPol[
        (val.alphaToIndex->polToInd[val.value] * power) % (val.gfOrder() - 1)],
            val.modPol, val.alphaToIndex);
}
...
public:
    ...
    explicit TableGFElem(const T& val, const T& modPol, GFtable* mulTable_, GFtable* divTable_)
        : BasicGFElem<T>(val, modPol) {
        mulTable = mulTable_;
        divTable = divTable_;
    }
    ...
    */
    ...
    GFtable* multable() { return mulTable; }
    GFtable* multable() const { return mulTable; }
    GFtable* divtable() { return divTable; }
    GFtable* divtable() const { return divTable; }
...
template <typename T>
class AccessorBasicIter;

/**
 * Storage for \c BasicGFElem<T> and all nested classes.
 */
template <class T>
class AccessorBasic {
public:
    using Elem = BasicGFElem<T>;
    using Iter = AccessorBasicIter<T>;

    AccessorBasic(): mEmpty(true) {}

    AccessorBasic(const Elem& elem)
        : mEmpty(false), mState(elem.getState()), mContained(decltype(Elem)) {
    ...
    void erase(const Elem& elem) {
        if (elem.getState() != mState)
            return; /// Correct result, you cannot erase an element from the storage that holds elements
                    /// of different types.

        mValues.erase(elem.val());
    }
    ...
    Iter construct(const T& value);
    Iter construct(const T& value);

private:
    bool mEmpty;
    std::type_index mContained;
